...
Detect forbidden cross-layer imports, adapter-to-adapter coupling, framework leakage into protected layers, bypassed ports/services, and new circular dependencies.

### AG-RESP — Module responsibility

Detect modules absorbing responsibilities outside their declared contract, orchestration modules accumulating domain decision rules, entrypoints becoming application kernels, and giant facades replacing shared services.

### AG-STATE — State ownership

Detect new mutable module globals, hidden singleton state, shared state without lifecycle ownership, and cache/session/config state introduced outside declared owners.

### AG-SIDE — Side-effect policy

Detect import-time or hidden filesystem writes, network calls, plugin registration, process/thread startup, environment mutation, and database/Redis/provider connection attempts.

### AG-DI — Dependency visibility

Detect core code that reaches directly into environment variables, global config, model/API clients, clocks/randomness, persistence sessions, or network providers when the contract requires explicit injection or ports.

### AG-CPLX — Complexity growth

Track soft/hard budgets for file length, outgoing dependency count, complexity, public responsibilities, fan-in/fan-out, and growth rate across changes.

### AG-API — Contract/interface change

Detect public API/schema changes, event/persistence format changes, migrations, breaking adapter contracts, and unversioned semantic reinterpretation.

### AG-TEST — Isolation and validation

Check whether changed core modules still have isolated tests, fake/in-memory providers remain available where required, architecture contract tests run, changed policy paths have rejection/failure tests, and behavior tests remain green.
...
Runs before write execution and asks whether the proposed implementation path is architecturally admissible, targets the correct modules, and requires a migration/ADR/exception first.

### Post-change Architecture Gate

Runs after implementation and asks whether actual code preserved admitted architecture, what changed structurally, and whether the implementation is eligible for review/merge.

A PASS at Plan Gate does not guarantee a PASS after implementation.

## 6. Baseline and drift

The gate maintains a project architecture baseline to distinguish pre-existing debt, newly introduced debt, debt reduced by the current change, and explicitly accepted exceptions.
...
The gate should report `existing_findings`, `introduced_findings`, and `resolved_findings` separately.
